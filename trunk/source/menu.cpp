...
static int QuickSaveOrLoad (int action)
{
	...
	{
		FILE *fp;
		MakeFilePath(filepath, FILE_SNAPSHOT, ROMFilename, saveNum);
	...
	if (action == 1) //save
	{
		return SaveBatteryOrState(filepath, FILE_SNAPSHOT, NOTSILENT);
	}
	else if (action == 0) // load
	{
		saveNum --;
		MakeFilePath(filepath, FILE_SNAPSHOT, ROMFilename, saveNum);
		return LoadBatteryOrState(filepath, FILE_SNAPSHOT, NOTSILENT);
	}
	return -1;
}
